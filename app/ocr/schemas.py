...
class OcrRegisterResponse(BaseModel):
    ...
    current_owner_name: str | None = Field(None, description="현재 소유자 성명")
    current_owner_address: str | None = Field(None, description="현재 소유자 주소 원문")
    current_owner_address_sigungu: str | None = Field(
        None, description="현재 소유자 주소에서 뽑은 시/군/구", examples=["경기 용인시"]
    )
    ownership_history: List[OwnershipEntry] = Field(
        default_factory=list, description="소유권 이전 이력 전체"
    )
    has_cancellation_mention: bool = Field(
        False, description="원문에 '말소' 언급이 있는지 여부"
    )
...
class RegisterCheckResponse(BaseModel):
    ...
    owner: OcrRegisterResponse = Field(..., description="등기부등본에서 뽑은 소유자 정보")
